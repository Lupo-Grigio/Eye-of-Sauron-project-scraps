...
JOYSTICK_Y_FLIP = True # If True, reverse stick Y axis
...
# INPUT CONFIG for eye motion controled by Stdin ----------------------------------------------
# ANALOG INPUTS REQUIRE SNAKE EYES BONNET

STDINCONTROL    = 1        # Boolean, are we controlling the eye gase from standard in
IDLE_FRAMES = 60           # how many input loops to process before idle
STDIN_X_IN = 0             # gase direction in the x axis
STDIN_Y_IN = 1             # gase direction in the y axis
STDIN_BLINK_IN = 2         # 0 or 1, Blink state, open or closed
STDIN_PUPILDIAL_IN = 3     # 0 to max, diameter of the pupil
STDIN_THREAD_PARAMS = 4    # Number of params passed from the tread
...
# Stdin stuff
# control via stdin paired tupls x,y --------------------------------------------------------------
# IF Stdin sends 0,0 or if Stdin is the same value for IDLE_FRAMES seconds
# eyes go into Idle mod

# Because  reads are blocking operations, and they take _forever_ comparatively
# So to prevent  input reads from bogging down the entire script,
# we read from input in a seporate thread. Just like ADC below.
# This thread read in control values thread and stores them in the global list adcValue[],
# which the animation loop can read at its leisure (with immediate results,
# no slowdown).

def StdinThread(adc, dest):
    ...
    while 1:
        ...
        if len(line) > 2 :
            integs = [int(num) for num in line.split(',')]
            if len(integs) == 2 :
                # print line + " Moving To " + str(integs[0]) + " , " + str(integs[1])
                # 0 to 1649-ish. are valid, clip:
                x = integs[STDIN_X_IN]
                y = integs[STDIN_Y_IN]
                if   x <    0: x =    0
                elif x > 1649: x = 1649
                ...
                dest[STDIN_X_IN] = x / 1649.0 # Store as 0.0 to 1.0
                dest[STDIN_Y_IN] = y / 1649.0 # Store as 0.0 to 1.0
                

#start reading from standard input if the variable STDINCONTROL == true
# at the moment we are using the same global variable, adcValue, as the ADC input code
# just a little bigger for Standard input
# TODO: Read blink state and pupil dialiation from Stdin
try:
    adcValue
except NameError:
    adcValue = [0] * STDIN_THREAD_PARAMS

if STDINCONTROL:
    thread.start_new_thread(StdinThread, (adc,adcValue)) 
...
dom               = parse("graphics/Sauron-eye3.svg")
vb                = getViewBox(dom)
...
#   IDLE DETECTION
    if STDINCONTROL :
        if adcValue[STDIN_X_IN] != lastX or adcValue[STDIN_Y_IN] != lastY :
            lastX = adcValue[STDIN_X_IN]
            lastY = adcValue[STDIN_Y_IN]
            SameCount = 0
        else : 
            SameCount = SameCount + 1

        if SameCount >= IDLE_FRAMES :
            input_idle = 1
        else :
            input_idle = 0
    

    if STDINCONTROL == 1 and input_idle == 0:
        ...
        curX = -30.0 + curX * 60.0
        curY = -30.0 + curY * 60.0       
    elif JOYSTICK_X_IN >= 0 and JOYSTICK_Y_IN >= 0:
        # Eye position from analog inputs
